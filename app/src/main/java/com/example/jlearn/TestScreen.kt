...
import com.example.jlearn.databinding.ActivityTestScreenBinding
...
class TestScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        wordCJ = settings.wordCJ
        wordJC = settings.wordJC
        exCJ = settings.exCJ
        exJC = settings.exJC
        numF = settings.numF
        numE = settings.numE
        searchCorrect = settings.searchCorrect
        searchCheck = settings.searchCheck
        howManyQue = settings.howManyQue
        howQue = settings.howQue
        oneMoreFlag = settings.oneMoreFlag
    ...
    private fun onNextTapped() {
        ...
        when (intentFlagToResult) {
            1 -> if (toResult == totalQue) {
                ...
                realm.executeTransaction {
                    realm.where<SettingsData>().findAll().deleteAllFromRealm()
                    ...
                    obj.howQue = howQue
                    obj.howManyCorrect = howManyCorrect
                    obj.howManyIncorrect = howManyIncorrect
                    obj.totalQue = totalQue
                }
            }
            2 -> {
                binding.next.visibility = View.INVISIBLE
